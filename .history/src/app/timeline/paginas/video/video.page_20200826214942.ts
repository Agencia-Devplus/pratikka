...
export class VideoPage implements OnInit {
  ...
  async postarVideo(videoURL, titulo: string) {
    const loading = await this.overlay.loading();
    // loading.present();

    // const path = videoURL.substring(videoURL.lastIndexOf('/') + 1, videoURL.indexOf('?'));
    const newPath = 'file:/' + videoURL;
    console.log('file:/' + videoURL);
    const buffer = await this.file.readAsArrayBuffer(newPath, titulo);
