...
export class VideoPage implements OnInit {
  ...
  async postarVideo(videoURL, titulo) {
    ...
    const path = videoURL.nativeURL.substr(
      0,
      videoURL.nativeURL.lastIndexOf('/') + 1
    );
    const type = this.getMimeType(videoURL.name.split('.').pop());
    const buffer = await this.file.readAsArrayBuffer(path, videoURL.name);
